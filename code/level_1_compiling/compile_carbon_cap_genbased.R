...
for ( i in 1:length(cases)){
  for (j in 1:length(years)){
    temp_emission_genratecap_fn <- paste0(RunFdr,"/",years[j],"/",case_ids[i],"_",years[j],"_",cases[i],"/Inputs/CO2GenRateCap.csv");
    if (file.exists(temp_emission_genratecap_fn)){
      ...
      temp_carbon_genratecap_budget <- temp_carbon_genratecap_raw %>%
        ...
        pivot_longer(temp_budgetcolumn, names_to = 'GenRateCapID',values_to = 'GenEmissionRate_Target_ton_per_MWh') %>%
      ...
      temp_carbon_genratecap_membership <- temp_carbon_genratecap_budget %>%
        ...
        pivot_longer(temp_membershipcolumn, names_to = 'GenRateCapID',values_to = 'Membership') %>%
        mutate(GenRateCapID = str_remove(GenRateCapID,'CO_2_Cap_Zone_'))
      temp_carbon_genratecap <- left_join(temp_carbon_genratecap_budget,temp_carbon_genratecap_membership, by = c('Region description','GenRateCapID'))
...
if(exists('carbon_genratecap')){
  ...
  rm(temp_emission_genratecap_fn, temp_carbon_genratecap_raw,temp_NoCap,temp_budgetcolumn,temp_carbon_genratecap_budget,temp_membershipcolumn,temp_carbon_genratecap_membership,temp_carbon_genratecap,carbon_genratecap)
