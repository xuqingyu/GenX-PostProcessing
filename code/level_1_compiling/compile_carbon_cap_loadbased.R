...
for ( i in 1:length(cases)){
  for (j in 1:length(years)){
    temp_emission_loadratecap_fn <- paste0(RunFdr,"/",years[j],"/",case_ids[i],"_",years[j],"_",cases[i],"/Inputs/CO2LoadRateCap.csv");
    if (file.exists(temp_emission_loadratecap_fn)){
      ...
      temp_carbon_loadratecap_membership <- temp_carbon_loadratecap_raw %>%
        ...
        pivot_longer(temp_membershipcolumn, names_to = 'LoadRateCapID',values_to = 'Membership') %>%
        mutate(LoadRateCapID = str_remove(LoadRateCapID,'CO_2_Cap_Zone_'))
      temp_carbon_loadratecap <- left_join(temp_carbon_loadratecap_budget,temp_carbon_loadratecap_membership, by = c('Region description','LoadRateCapID'))
...
if(exists('carbon_loadratecap')){
  ...
  rm(temp_emission_loadratecap_fn, temp_NoCap,temp_budgetcolumn,temp_carbon_loadratecap_raw,temp_carbon_loadratecap_budget,temp_membershipcolumn,temp_carbon_loadratecap_membership,temp_carbon_loadratecap)
