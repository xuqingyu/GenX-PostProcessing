...
for ( i in 1:length(cases)){
  for (j in 1:length(years)){
    temp_LSETransLossCost_fn <- paste0(RunFdr,"/",years[j],"/",case_ids[i],"_",years[j],"_",cases[i],"/Results/TransmissionLossCost.csv");
    if (file.exists(temp_LSETransLossCost_fn)){
      temp_LSETransLossCost = read.csv(temp_LSETransLossCost_fn)
