...
library(tidyverse)
folder <- "~/Documents/WECC_Google_newruns_it2/CompiledResults/"
...
system_cost <- read_csv(paste0(folder,'California/System_Cost_long_California.csv')) %>%
  group_by(case,year,Scenario, TechSensitivity) %>%
  summarise(value = sum(value)) %>%
...
write_csv(reference, 
          "/Users/qingyuxu/Dropbox (Princeton)/PJM/Project_GenX_OutputProcessing/data/Google_WECC_CFEStudy_Reference.csv")  
