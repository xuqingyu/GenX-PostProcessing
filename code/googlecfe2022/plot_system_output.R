...
cfe_output_allcase_system <- read_csv(paste0(RunFdr,"/CompiledResults/", 
...
cfe_output_allcase_system_ref <- read_csv(paste0(RunFdr,"/CompiledResults/", 
                                                 subreg,"/Generation/Gen_Output_",subreg,".csv"),
                                          col_types = cols()) %>%
  mutate(Fuel = factor(Fuel, levels = capacity_resource_levels)) %>%
  na.omit() %>%
...
if (Studyregion == 'PJM') {
  troublerows = which(cfe_output_allcase_system_diff$TechSensitivity == 'Annual 100%' & 
                        cfe_output_allcase_system_diff$Scenario == '10% CI Part., Adv. Tech. no Comb.')
  cfe_output_allcase_system_diff$Target[troublerows] <- 0.74
  captiontext = "Dots denote the volume of increased exports to the rest of the study region"
}
if (Studyregion == 'WECC') {
  captiontext = "Dots denote the volume of displaced imports from the rest of WECC"
}
...
ggplot()+
  ...
  labs(x = 'CFE Score', y = 'Generation Output Diff (TWh)', caption = captiontext)+ 
  ...
  theme_bw() +
  guides(fill = guide_legend(title = NULL, title.position = NULL))+
  theme(legend.position = "bottom",
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"),
        legend.title = element_blank()) + 
  ggsave(paste0(RunFdr,'/CompiledResults/',subreg,
                '/Graphics/',Studyregion,'_System_Output_Compared_to_noPurchase_10p_new.png'),
         width = 7,
         height = 10)
...
ggplot(data = filter(cfe_output_allcase_system_ref, 
  ...
  ggsave(paste0(RunFdr,'/CompiledResults/',subreg,
                '/Graphics/',Studyregion,'_System_Output_Reference_10p.png'),
         width = 1,
         height = 4)
ggplot()+
  geom_col(data = filter(cfe_output_allcase_system_diff, 
  ...
  theme(legend.position = "bottom",
        ...
        legend.title = element_blank()) + 
  ggsave(paste0(RunFdr,'/CompiledResults/',subreg,
                '/Graphics/',Studyregion,'_System_Output_Compared_to_noPurchase_25p_new.png'),
         width = 7,
         height = 10)
