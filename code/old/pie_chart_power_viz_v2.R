...
dir.create(paste0(RunFdr,"/Graphics/power_pie_charts"), showWarnings = FALSE)

Power_csv <- read_csv(paste0(RunFdr,"/CompiledResults/power.csv"));
  ...
  for (j in 1:length(years)) {
    ...
    PJM_total_and_West$case = 
     ...
     ggplot(PJM_total_and_West, aes(x=total/2, y = value, fill = factor(Fuel, levels = fuel_list), width = total)) +
      ...
      ggsave(paste0(RunFdr,"/Graphics/power_pie_charts/", years[j], "_power_PJM_bubble.png"), width=10, height=10, dpi=300)
    ...
    ggplot(all_NJ, aes(x=total/2, y = value, fill = factor(Fuel, levels = fuel_list), width = total)) +
      ...
      labs(y= paste0(years[j], "_", cases), x="Annual Energy (TWh)")+
      theme(axis.text.x = element_blank()) +
      ggsave(paste0(RunFdr,"/Graphics/power_pie_charts/", years[j], "_power_NJ_bubble.png"), width=10, height=10, dpi=300)
...
ggplot(PJM_power, aes(x=total/2, y = value, fill = factor(Fuel, levels = fuel_list), width = total)) +
  ...
  ggsave(paste0(RunFdr,"/Graphics/power_pie_charts/", "total_power_PJM_bubble.png"), width=10, height=10, dpi=300)
