...
cfe_capacity_allcase <- read_csv(paste0(RunFdr,"/CompiledResults/",subreg,
                                        "/Generation/tfs_gen_capacity_",subreg,".csv")) %>%
...
cfe_capacity_allcase$Target[Annualrows] <- cfe_capacity_allcase$`Post-Grid CFE Score Local_n_Import`[Annualrows]
if (Studyregion == 'PJM') {
  troublerows = which(cfe_capacity_allcase$TechSensitivity == 'Annual 100%' & 
                        cfe_capacity_allcase$Scenario == '10% CI Part., Adv. Tech. no Comb.')
  cfe_capacity_allcase$Target[troublerows] <- 0.74
}
cfe_capacity_allcase <- cfe_capacity_allcase %>%
  arrange(Target)
...
if (Studyregion == 'WECC') {
  ...
  cap_limit_10p = c(0,55)
  cap_break_10p = seq(from = 0, to = 55, by = 10)
