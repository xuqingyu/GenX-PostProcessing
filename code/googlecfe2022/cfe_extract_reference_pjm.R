...
library(tidyverse)
folder <- "~/Documents/PJM_247/CompiledResults/"
ci_emission <- read_csv(paste0(folder,'tfs_part_emissions.csv'), col_types = cols()) %>%
  select(case,year,Scenario, TechSensitivity, Policy, emission_measure, `Participated Load`) %>%
...
system_cost <- read_csv(paste0(folder,'PJM_Google/System_Cost_long_PJM_Google.csv'), col_types = cols()) %>%
...
write_csv(reference, 
          "/Users/qingyuxu/Dropbox (Princeton)/PJM/Project_GenX_OutputProcessing/data/Google_PJM_CFEStudy_Reference_2022.csv")  
