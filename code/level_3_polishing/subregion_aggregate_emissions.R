...
for ( i in 1:n_subregions){
  ...
  total_load_subregion <- read_csv(lse_payment_subregion_fn, col_types = cols()) %>%
  ...
  if (file.exists(emissions_subregion_fn)) {
    ...
    if ((temp_total_title == 'New Jersey')|(temp_total_title == 'PJM')) {
      if (temp_total_title == 'New Jersey'){
        emissions_2019_base <- read_csv('./data/Emissions_2019_NJ.csv', col_types = cols())
      } else if (temp_total_title == 'PJM'){
        emissions_2019_base <- read_csv(paste0(RunFdr,'/Emissions_2019_PJM.csv'), 
                                        col_types = cols())
      }
