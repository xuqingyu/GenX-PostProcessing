...
if (Studyregion == 'WECC') {
  ci_load_before_time_reduction = 193.74 
  cfe_gen_allcase = cfe_gen_allcase %>%
  ...
  ci_load_before_time_reduction = 557.60
...
ggplot(data = filter(cfe_gen_allcase_total, 
  ...
  scale_y_continuous(limits = c(0,1.4),breaks = seq(0,1.5,0.1)) +
  ...
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_10p_new_norm.png'),
         width = 6,
         height = 6)
...
ggplot(data = filter(cfe_gen_allcase, 
  ...
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_10p_new.png'),
         width = 15,
         height = 5)
...
ggplot(data = filter(cfe_gen_allcase, 
  ...
  theme_bw() +
  theme(legend.position = 'bottom')+
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_10p_all.png'),
         width = 12,
         height = 9)
ggplot(data = filter(cfe_gen_allcase, 
                     grepl('^10%',Scenario),
  ...
  theme_bw() +
  theme(legend.position = 'bottom')+
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_10p.png'),
         width = 4,
         height = 9)
...
ggplot(data = filter(cfe_gen_allcase, 
  ...
  theme_bw() +
  theme(legend.position = 'bottom')+
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_5p.png'),
         width = 4,
         height = 9)
ggplot(data = filter(cfe_gen_allcase, 
                     grepl('^25%',Scenario),
  ...
  theme_bw() +
  theme(legend.position = 'bottom')+
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_25p.png'),
         width = 4,
         height = 9)
...
#   theme_bw() +
#   theme(legend.position = 'bottom')+
#   ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_10p_ces.png'),
#          width = p_width_all,
#          height = p_height_all)
# 
# ggplot(data = filter(cfe_gen_allcase, 
...
#          height = p_height_all)
# 
cfe_gen_allcase_all100 <- read_csv(paste0(temp_RunFdr,"/CompiledResults/",subreg,"/Generation/CFE_Gen_Output_",subreg,".csv")) %>%
  mutate(Fuel = factor(Fuel, levels = capacity_resource_levels)) %>%
  left_join(cfe_score, by = c('case','year','Scenario','TechSensitivity')) %>%
  mutate(TechSensitivity = factor(TechSensitivity, levels = x_list)) %>%
  mutate(Scenario = factor(Scenario, levels = y_list)) %>%
...
ggplot(data = filter(cfe_gen_allcase_all100,
  ...
  theme_bw() +
  theme(legend.position = 'bottom')+
  ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_annual100_10p.png'),
         width = 4,
         height = 9)
...
#   theme_bw() +
#   theme(legend.position = 'bottom')+
#   ggsave(paste0(temp_RunFdr,'/CompiledResults/',subreg,'/Graphics/CFE_Output_annual100_10p_ces.png'),
#          width = p_width_ref,
#          height = p_height_ref)
# 
# ggplot(data = filter(cfe_gen_allcase_all100,
