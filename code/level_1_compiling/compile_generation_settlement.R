...
if (exists('Settlement')){
  ...
  # capacity_for_settlement <- 
  #   read_csv(paste0(RunFdr,"/CompiledResults/capacity_for_settlement.csv"), 
  #            col_types = cols()) %>%
  #   mutate(year = as.factor(year));
  # Settlement <- left_join(Settlement,capacity_for_settlement,
  #                         by = c("Region","Resource","Fuel", 
  #                                "Zone","Cluster","case","year"));
  # 
  # power_for_settlement <- 
  #   read_csv(paste0(RunFdr,"/CompiledResults/power_for_settlement.csv"), 
  #            col_types = cols()) %>%
  #   mutate(year = as.factor(year));
  # Settlement <- left_join(Settlement,power_for_settlement,
  #                         by = c("Region","Resource","Fuel", 
  #                                "Zone","Cluster", "case","year"))
  # 
  # charge_for_settlement <- 
  #   read_csv(paste0(RunFdr,"/CompiledResults/charge_for_settlement.csv"), 
  #            col_types = cols()) %>%
  #   mutate(year = as.factor(year)) %>%
  #   rename(ChargeSum = Sum);  
  # Settlement <- left_join(Settlement,charge_for_settlement, 
  #                         by = c("Region","Resource","Fuel", 
  #                                "Zone","Cluster", "case","year"))
