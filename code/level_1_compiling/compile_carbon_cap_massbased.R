...
for ( i in 1:length(cases)){
  for (j in 1:length(years)){
    temp_emission_masscap_fn <- paste0(RunFdr,"/",years[j],"/",case_ids[i],"_",years[j],"_",cases[i],"/Inputs/CO2Cap.csv");
    if (file.exists(temp_emission_masscap_fn)){
      ...
      temp_carbon_masscap_budget <- temp_carbon_masscap_raw %>%
        ...
        pivot_longer(temp_budgetcolumn, names_to = 'MassCapID',values_to = 'Mass_Budget_Mtons') %>%
      ...
      temp_carbon_masscap_membership <- temp_carbon_masscap_raw %>%
        ...
        pivot_longer(temp_membershipcolumn, names_to = 'MassCapID',values_to = 'Membership') %>%
        mutate(MassCapID = str_remove(MassCapID,'CO_2_Cap_Zone_'))
      temp_carbon_masscap <- left_join(temp_carbon_masscap_budget,temp_carbon_masscap_membership, by = c('Region description','MassCapID'))
...
if(exists('carbon_masscap')){
  ...
  rm(carbon_masscap,temp_carbon_masscap, temp_emission_masscap_fn,temp_carbon_masscap_raw, temp_NoCap,temp_budgetcolumn,temp_carbon_masscap_budget,temp_membershipcolumn,temp_carbon_masscap_membership)
