    ...
    for (j in 1:n_comparison){
      ...
      for (k in 1:length(interested_sensitivity)) {
        ...
        ggplot()+
          ...
          ggsave(paste0(RunFdr,'/CompiledResults/',Subregions[i],'/Graphics/GenCapacity/GenCapacity_',comparison[j],'_',temp_total_title,'_',k,'.png'),width = 7.5,height=7)
      }
    }
      ...
      for (k in 1:length(interested_scenario)){
        ...
        if (dim(temp_gen_capacity_compared_data)[1]>0){
          ...
          ggdraw() +
            ...
            ggsave(paste0(RunFdr,'/CompiledResults/',Subregions[i],'/Graphics/GenCapacity/Generation Capacity (GW)_Sensitivity Comparison_', j ,"_of_", temp_total_title,"_of_", interested_scenario[k],'.png'),width = 9,height=7)
        }
      }
    }
  }
