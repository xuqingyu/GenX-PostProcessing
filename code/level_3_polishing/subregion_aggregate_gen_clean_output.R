...
for (i in 1:n_subregions) {
  ...
  gen_output_fn <- paste0(RunFdr,'/CompiledResults/',Subregions[i],
                          '/Generation/Gen_Output_',temp_total_title,".csv")
  temp_re_output <- read_csv(gen_output_fn, col_types = cols()) %>%
  ...
  temp_ces_output <- read_csv(gen_output_fn, col_types = cols()) %>%
