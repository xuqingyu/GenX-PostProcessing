...
cfe_capacity_allcase_system <- read_csv(paste0(RunFdr,"/CompiledResults/",subreg,
...
cfe_capacity_allcase_system_ref <- read_csv(paste0(RunFdr,"/CompiledResults/",subreg,
                                                   "/Generation/Gen_Capacity_",subreg,".csv"),
                                            col_types = cols()) %>%
  mutate(Fuel = factor(Fuel, levels = capacity_resource_levels)) %>%
  na.omit() %>%
...
if (Studyregion == 'PJM') {
  troublerows = which(cfe_capacity_allcase_system_diff$TechSensitivity == 'Annual 100%' & 
                        cfe_capacity_allcase_system_diff$Scenario == '10% CI Part., Adv. Tech. no Comb.')
  cfe_capacity_allcase_system_diff$Target[troublerows] <- 0.74
  # captiontext = "Dots denote the volume of displaced exports to the rest of the study region"
}
if (Studyregion == 'WECC') {
...
ggplot(data = filter(cfe_capacity_allcase_system_diff, 
  ...
  ggsave(paste0(RunFdr,'/CompiledResults/',subreg,'/Graphics/',subreg,'_System_Capacity_Compared_to_noPurchase_10p_new.png'),
         width = 7,
         height = 10)
ggplot(data = filter(cfe_capacity_allcase_system_ref, 
  ...
  ggsave(paste0(RunFdr,'/CompiledResults/',subreg,'/Graphics/',subreg,'_System_Capacity_Reference_10p.png'),
         width = 1,
         height = 4)
