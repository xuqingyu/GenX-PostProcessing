...
for ( i in 1:length(cases)){
  for (j in 1:length(years)){
    temp_TransCongestionRevenue_fn <- paste0(RunFdr,"/",years[j],"/",
                                             case_ids[i],"_",years[j],"_",
                                             cases[i],
                                             "/Results/CongestionRevenue.csv");
    if (file.exists(temp_TransCongestionRevenue_fn)){
      ...
      if(!exists('TransCongestionRevenue')) {
        ...
        TransCongestionRevenue <- rbind(TransCongestionRevenue, 
                                        temp_TransCongestionRevenue);
      }
    }
  }
}
if (exists('TransCongestionRevenue')) {
  write_csv(TransCongestionRevenue, 
            paste0(RunFdr,"/CompiledResults/TransCongestionRevenue.csv"));
